...
class ChessGameMode:
    ...
    def move_piece_at_pos(self, match, start, end, promotion_type=PieceType.QUEEN):
        ...
        if end in moves:
            ...
            self._update_match_state(start, end, match, piece_num, type)
        else:
    ...
    def resolve_game_state(self, match, is_white_turn, promotion_type):
        """Determines whether the specified match is over yet."""
        white_king_pos = board_utils.find_piece_pos(6, match.board)
        white_has_moves = self.player_has_moves(match, True, promotion_type)
        if self.is_in_check(white_king_pos, True, match):
            if white_has_moves:
        ...
        black_has_moves = self.player_has_moves(match, False, promotion_type)
        if self.is_in_check(black_king_pos, False, match):
            if black_has_moves:
    ...
    def player_has_moves(self, match, is_white, promotion_type):
        """Checks whether the given player has moves for this match."""
        moves = self.get_player_moves(match, is_white, promotion_type)
    ...
    def get_player_moves(self, match, is_white, promotion_type):
        ...
        indexes = self._get_player_piece_indexes(match.board, is_white)

        total_moves = {}
        for index in indexes:
            piece_pos = board_utils.get_pos_from_index(index)
            piece_value = board_utils.get_piece_at_pos(match.board, piece_pos)
            piece = self.get_piece_object(piece_value)
            all_moves = piece.get_valid_moves(piece_pos, match)
            safe_moves = self._remove_friendly_check_moves(all_moves, piece_pos, promotion_type, match)
            total_moves[piece_pos] = safe_moves
        return total_moves

    
    def _get_player_piece_indexes(self, board, is_white):
